...
func populate_buttons(bench: Node3D):
	...
	button1.text = bench_mons[0].display_name + " (AV: %s, RV: %s, HP: %s)" % [bench_mons[0].action_value, bench_mons[0].rv, bench_mons[0].hp]
	button2.text = bench_mons[1].display_name + " (AV: %s, RV: %s, HP: %s)" % [bench_mons[1].action_value, bench_mons[1].rv, bench_mons[1].hp]
